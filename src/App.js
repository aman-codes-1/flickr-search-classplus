...
export default class App extends React.Component {
	...
	componentDidMount() {
		...
		this.makeDebouncedSearch = debounce(() => {
			...
			fetch(url)
				...
				.then(resp => {
					...
					this.setState({ imageList: picArray });
				})
				.catch(err => {
					console.log(err);
				});
		...
		fetch(constants.GET_RECENT_URL)
			.then(checkHttpStatus)
			.then(parseJSON)
			.then(function(resp){
				let picArray = resp.photos.photo.map((pic) => {
		...
		return (
			<div className="app">
				<div className="app-header">
					<h2 style={{ margin: "1rem 0" }}>Flickr Search Tool</h2>
				...
				<div className="app-content" ref="appContent">
					{this.state.imageList.length
						...
						: <p style={{ margin: "1rem 0" }}>Try searching for some images in the search bar</p>
					}
